...
def parse_sqlite(db_path, selected_date):
    ...
    try:
        ...
        query = f"""
        SELECT 
            strftime('%Y-%m-%d %H:%M', datetime(TS / 1000000, 'unixepoch', 'UTC', '+4 hours')) AS TS,
            Val1 AS Val1,
            Val2 AS Val2,
            Val3 AS Val3
        FROM 
            TblTrendData
        WHERE 
            datetime(TS / 1000000, 'unixepoch', 'UTC', '+4 hours') BETWEEN '{start_date}' AND '{end_date}'
        GROUP BY 
            CAST(strftime('%s', datetime(TS / 1000000, 'unixepoch', 'UTC', '+4 hours')) / (1 * 60) AS INTEGER)
        ORDER BY 
            TS;"""
